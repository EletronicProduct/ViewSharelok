document.addEventListener('DOMContentLoaded', () => {
  ...
  const app = new Vue({
    ...
    data: () => ({
      ...
      markers: {},
      polylines: {},
      accuracyCircles: {}, // Untuk menyimpan lingkaran akurasi
      db: null,
    ...
    methods: {
      ...
      monitorUsers() {
        ...
        this.db.ref('latest').on('value', snapshot => {
          ...
          const userArray = Object.keys(data).map(key => ({
            ...
            // Pastikan data heading/bearing dikirim dari aplikasi user
            heading: data[key].heading || 0, // Default 0 jika tidak ada
            accuracy: data[key].accuracy || 0, // Default 0 jika tidak ada
            ...data[key]
          }));
        ...
        this.db.ref('tracking').on('child_added', snapshot => {
          const userId = snapshot.key;
          this.db.ref(`tracking/${userId}`).on('child_added', updateSnapshot => {
      ...
      updateMapMarkers() {
        this.users.forEach(user => {
          ...
          const customIcon = L.divIcon({
            className: 'custom-marker-div-icon', // Nama class untuk styling
            html: customIconHtml,
            iconSize: [20, 30], // Ukuran div icon (lebar, tinggi)
            iconAnchor: [10, 30] // Poin mana icon harus menempel di koordinat
          });
          
          if (this.markers[user.id]) {
            // Update posisi marker
            this.markers[user.id].setLatLng(latLng);
            // Update icon marker untuk mengubah arah panah
          ...
          // Update atau buat lingkaran akurasi
          if (user.accuracy > 0) {
          ...
          this.reverseGeocode(user.lat, user.lng).then(address => {
            ...
            if (userIndex !== -1) {
              this.$set(this.users[userIndex], 'address', address); // Pastikan reaktif
            }
            // Perbarui pop-up dengan alamat baru
            if (this.markers[user.id]) {
      ...
      panToUser(user) {
        if (user.lat && user.lng) {
          ...
          // Opsional: Buka pop-up marker ketika di-pan
          if (this.markers[user.id]) {
